...
class AddTractorPage extends StatefulWidget {
  ...
  @override
  _AddTractorPageState createState() => _AddTractorPageState();
}
...
class _AddTractorPageState extends State<AddTractorPage> {
  ...
  final minio = Minio(
    endPoint: 'CONTABO_ENDPOINT', // Example: 'eu2.contabostorage.com'
    accessKey: 'CONTABO_ACCESS_KEY',
    secretKey: 'CONTABO_SECRET_KEY',
    useSSL: true, // Contabo supports HTTPS
  );
  ...
  Future<String?> _uploadToContabo(File file) async {
    try {
      ...
      String uploadedUrl =
          'https://CONTABO_ENDPOINT/CONTABO_BUCKET/$fileName';
  ...
  Future<void> addTractor() async {
    try {
      ...
      final tractorData = {
        ...
        "description": "",
        "insuranceStatus": widget.InStatus.text.trim(),
        "listingDate": DateTime.now().toUtc().toIso8601String(),
        "pincode": widget.pincode.text.trim(),
        "rearTyre": widget.RearTyre.text.trim(),
        "sellPrice": widget.amount.text.trim(),
        "status": "pending",
        "images": uploadedUrls, // Store uploaded images array
      };
  ...
  Widget build(BuildContext context) {
    return Scaffold(
      appBar: AppBar(title: const Text("Upload Tractor Images")),
      body: Column(
        children: [
          ElevatedButton(
          ...
          Expanded(
            child: ListView.builder(
              ...
              itemBuilder: (context, index) {
                return ListTile(
                  leading: Image.network(uploadedUrls[index], width: 50, height: 50, fit: BoxFit.cover),
                  title: Text(uploadedUrls[index]),
                );
              },
            ),
          ),
          ElevatedButton(
            onPressed: uploadedUrls.isNotEmpty ? addTractor : null,
...
//
//   // Contabo S3 Credentials
//   final String accessKey = 'CONTABO_ACCESS_KEY';
//   final String secretKey = 'CONTABO_SECRET_KEY';
//   final String bucketName = 'CONTABO_BUCKET';
//   final String region = 'CONTABO_REGION';
//   final String endpoint = 'https://CONTABO_ENDPOINT';
//
//   Future<void> _pickImages() async {
//     final List<XFile>? pickedFiles = await _picker.pickMultiImage();
//     if (pickedFiles == null) return;
...
//       String fileName = 'uploads/${DateTime.now().millisecondsSinceEpoch}.jpg';
//
//       String uploadedUrl = await AmazonS3.uploadFile(
//         file.path,
//         bucketName,
//         region,
//         accessKey,
...
//   Widget build(BuildContext context) {
//     return Scaffold(
//       appBar: AppBar(title: Text("Upload Tractor Images")),
//       body: Column(
//         children: [
//           ElevatedButton(
//             onPressed: _showImageSourceDialog,
